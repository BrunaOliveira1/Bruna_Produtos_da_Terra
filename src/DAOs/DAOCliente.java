...
public class DAOCliente extends DAOGenerico<Cliente> {
    ...
    public List<Cliente> listByNome(String nome) {
        return em.createQuery("SELECT e FROM Cliente e WHERE e.nomeCompleto LIKE :nome").setParameter("nome", "%" + nome + "%").getResultList();
    }
    ...
    public List<Cliente> listInOrderNome() {
        return em.createQuery("SELECT e FROM Cliente e ORDER BY e.nomeCompleto").getResultList();
    }
    ...
    public List<String> listInOrderNomeStrings(String qualOrdem) {
        ...
        for (int i = 0; i < lf.size(); i++) {
            ls.add(lf.get(i).getCpf() + "-" + lf.get(i).getNomeCompleto());
        }
        return ls;
    }
    ...
    public static void main(String[] args) {
        ...
        for (Cliente cliente : listaCliente) {
            System.out.println(cliente.getCpf() + "-" + cliente.getNomeCompleto());
        }
    }
}
