...
public class DAOVenda extends DAOGenerico<Venda> {
    ...
    public List<Venda> listByNome(String nome) {
        return em.createQuery("SELECT e FROM Venda e WHERE e.cadastroClienteCPF LIKE :nome").setParameter("nome", "%" + nome + "%").getResultList();
    }
    ...
    public List<Venda> listInOrderNome() {
        return em.createQuery("SELECT e FROM Venda e ORDER BY e.cadastroclienteCPF").getResultList();
    }
    ...
    public List<String> listInOrderNomeStrings(String qualOrdem) {
        ...
        for (int i = 0; i < lf.size(); i++) {
            ls.add(lf.get(i).getIdVenda() + "-" + lf.get(i).getCadastroclienteCPF());
        }
        return ls;
    }
    ...
    public static void main(String[] args) {
        ...
        for (Venda venda : listaVenda) {
            System.out.println(venda.getIdVenda() + "-" + venda.getCadastroclienteCPF());
        }
    }
}
