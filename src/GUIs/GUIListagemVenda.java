...
public class GUIListagemVenda extends JDialog {
    ...
    public GUIListagemVenda(List<Venda> texto) {
        ...
        String[] colunas = new String[]{"Id da Venda", "CPF do Cliente",};
        String[][] dados = new String[0][3];
        DefaultTableModel model = new DefaultTableModel(dados, colunas);
        JTable tabela = new JTable(model);
        scroll.setViewportView(tabela);
        for (int i = 0; i < texto.size(); i++) {
            String[] linha = new String[]{String.valueOf(texto.get(i).getIdVenda()), String.valueOf(texto.get(i).getCadastroclienteCPF()),};
            model.addRow(linha);
        }
            painelTa.add(scroll);
